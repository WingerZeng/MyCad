...
namespace vrt{
	...
	vrt::bpFace* mef(bpVertex* v1, bpVertex* v2, bpLoop* lp)
	{
	...
	vrt::bpFace* mef(bpHalfEdge* pe1, bpHalfEdge* pe2, bpLoop*lp)
	{
		if (!pe1 || !pe2 || !lp) {
			assert(0);
			return nullptr;
	...
	vrt::bpFace* kemr(bpLoop* lp, bpHalfEdge* he1)
	{
		if (!lp || !he1) {
			assert(0);
			return nullptr;
	...
	vrt::bpSolid* createFaceFromLoop(const std::vector<std::vector<Point3f>>& lps_, bpFace** pface, bpFace** prface)
	{
		auto lps = lps_;
