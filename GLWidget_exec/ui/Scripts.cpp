...
namespace vrt{
	...
	void ScriptList::scriptToFile(Script scp, QString filePath)
	{
		...
		fout << "#title" << std::endl;
		fout << scp.title.toStdString() << std::endl;
		fout << "#content" << std::endl;
		fout << scp.content.toStdString() << std::endl;
		fout << "#description" << std::endl;
		fout << scp.description.toStdString() << std::endl;
		return;
	}

	vrt::Script ScriptList::fileToScript(QString filePath, bool* isok)
	{
		std::ifstream fin(filePath.toStdString());
		std::string str;
		std::stringstream sin;
		sin << fin.rdbuf();
		QString qstr = QString::fromStdString(sin.str());
	...
	{
		QDirIterator qdi(PROPTR->swPath() + "/scripts",
			QStringList() << "*.scp",
