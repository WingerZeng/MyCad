...
namespace vrt{
	...
	int CadInterface::mvfs(const Point3f& coord)
	{
		bpLoop* nlp;
		bpVertex* nvtx;
		bpSolid* nsld = vrt::mvfs(coord, &nlp,&nvtx);
		addFace(nlp->Lface());
		addLoop(nlp);
		addVertex(nvtx);
	...
	CadInterface::HalfEdgeHandle CadInterface::mev(VertexHandle vertex, LoopHandle lp, const Point3f& coord)
	{
		...
		bpHalfEdge* nhe = vrt::mev(pVtx, plp, coord);
	...
	int CadInterface::kfmrh(FaceHandle f1, FaceHandle f2)
	{
		...
		SolidHandle sld1 = findSolid(vrt::getSolid(pf1));
		SolidHandle sld2 = findSolid(vrt::getSolid(pf2));
		int ret = vrt::kfmrh(pf1, pf2);
	...
	vrt::CadInterface::FaceHandle CadInterface::kemr(LoopHandle lp, HalfEdgeHandle he)
	{
		bpLoop* plp = findLoop(lp);
		...
		CHECK(plp&&phe);
		FaceHandle fcid = findFace(vrt::kemr(plp, phe));
		//#TODO info output
		if (fcid != ErrorHandle) {
			delHalfEdge(he1);
			delHalfEdge(he2);
	...
	int CadInterface::sweep(SolidHandle solid, FaceHandle face, Vector3f dir, Float dist)
	{
		...
		CHECK(psld&&pfc);
		int ret = vrt::sweep(psld, &pfc, dir, dist);
		updateSolid(solid);
	...
	void CadInterface::updateFace(FaceHandle face)
	{
		...
		if (it == handle2Item.end()) {
			...
			handle2Item[findSolid(vrt::getSolid(pfc))]->addChild(faceItem);
	...
	void CadInterface::updateLoop(LoopHandle loop)
	{
		...
		if (!plp->getFirstHalfEdge()) { //如果loop没有半边，说明loop是mvfs生成的
			QTreeWidgetItem* vitem = new QTreeWidgetItem({ "Vertex",QString::number(findVertex(vrt::getSolid(plp)->getVertex())) });
			handle2Item[findVertex(vrt::getSolid(plp)->getVertex())] = vitem;
...
#undef DEF_ELEM_SELECTION_CHANGE_FUNC
	...
	vrt::CadInterface::SolidHandle CadInterface::getSolid(ELEMEMNT_TYPE type, ElemHandle elem)
	{
		switch (type)
		{
		case vrt::CadInterface::SOLID:
			return elem;
			break;
		case vrt::CadInterface::FACE:
			return findSolid(vrt::getSolid(findFace(elem)));
			break;
		case vrt::CadInterface::LOOP:
			return findSolid(vrt::getSolid(findLoop(elem)));
			break;
		case vrt::CadInterface::HALFEDGE:
			CHECK(0);
			break;
		case vrt::CadInterface::VERTEX:
			CHECK(0);
			break;
		default:
			CHECK(0);
	...
	vrt::CadInterface::ElemHandle CadInterface::item2Handle(QTreeWidgetItem* item)
	{
		return item->text(1).toInt();
	}

	vrt::CadInterface::SolidHandle CadInterface::createFaceFromLoop(const std::vector<std::vector<Point3f>>& lps)
	{
		if (lps.empty()) return ErrorHandle;
		bpSolid* sld = vrt::createFaceFromLoop(lps);
	...
	std::vector<vrt::CadInterface::HalfEdgeHandle> CadInterface::getHalfEdgesOfLoop(LoopHandle lp)
	{
		...
		std::vector<vrt::CadInterface::HalfEdgeHandle> result;
		bpHalfEdge* he = fhe;
		do {
	...
	std::vector<std::shared_ptr<vrt::Primitive>> CadInterface::SolidToPrimitives(SolidHandle solidid)
	{
		auto solid = findSolid(solidid);
		bpFace* fc = solid->getFace();
		std::vector<std::shared_ptr<vrt::Primitive>> plgs;
	...
	std::vector<std::shared_ptr<vrt::Primitive>> CadInterface::FaceToPrimitives(FaceHandle faceid)
	{
		...
		if(lps.size()) plgs.emplace_back(new vrt::PPolygon(lps));
		return plgs;
	}

	std::vector<std::shared_ptr<vrt::Primitive>> CadInterface::HalfEdgeToPrimitives(HalfEdgeHandle heid)
	{
		auto he = findHalfEdge(heid);
		std::vector<Point3f> pts;
		pts.push_back(he->getBeginVtx()->getCoord());
		pts.push_back(he->getEndVtx()->getCoord());
		return { std::shared_ptr<vrt::Primitive>(new PLines(pts,false)) };
	}

	std::vector<std::shared_ptr<vrt::Primitive>> CadInterface::LoopToPrimitives(LoopHandle lpid)
	{
		auto lp = findLoop(lpid);
		auto he = lp->getFirstHalfEdge();

		if (!he) {
			return { std::shared_ptr<vrt::Primitive>(new PPoint(vrt::getSolid(lp)->getVertex()->getCoord())) }; // loop没有边，说明是mvfs生成的
		}
		...
		pts.push_back(he->getBeginVtx()->getCoord());
		return { std::shared_ptr<vrt::Primitive>(new PLines(pts,true)) };
	}

	std::vector<std::shared_ptr<vrt::Primitive>> CadInterface::VertexToPrimitives(VertexHandle vid)
	{
		auto v = findVertex(vid);
		return { std::shared_ptr<vrt::Primitive>(new PPoint(v->getCoord())) };
	}
